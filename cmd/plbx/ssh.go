...
func newSSHProxyCmd(g *globals) *cobra.Command {
	return &cobra.Command{
		...
		Long: "Connect ssh to a sandbox. This is what `plbx setup ssh` puts in your " +
			"ProxyCommand, and it is not useful to run by hand.\n\n" +
			"It writes the sandbox's name to plbx's ssh socket and then relays bytes. " +
...
func sshProxy(cmd *cobra.Command, g *globals, host string) error {
	...
	if err != nil {
		return fmt.Errorf("connecting to plbx's ssh gateway: %w", err)
	}
...
func newSetupSSHCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Add a managed block to your ssh config, so that `ssh myrepo." + api.SSHDomain + "` " +
			...
			"onto plbx's own socket, which only you can open.\n\n" +
