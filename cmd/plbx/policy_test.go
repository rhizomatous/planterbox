...
func TestReadingThePolicyNeverWritesOne(t *testing.T) {
	...
	// that decides it. A question in the way of `plbx policy ls` answers
...
func TestAnUnwrittenPolicyIsNotASpecialCase(t *testing.T) {
	...
	// what the proxy enforces. Whether a file exists yet is plbx's business,
...
func TestEditingWithNoPolicyStartsFromTheDefault(t *testing.T) {
	// the answer to `plbx policy allow x` is to allow x, not to ask which
