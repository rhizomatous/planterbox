...
// the current directory so `jard stop` works from inside a workspace the same
// way `jard run` does.
...
func newExecCmd(g *globals) *cobra.Command {
	...
	// stop parsing flags at the first positional, or `jard exec box bash -lc ...`
	// has its -lc read as jard's own. Flags go before the sandbox name, exactly
...
func newAgentsCmd(_ *globals) *cobra.Command {
	return &cobra.Command{
		Use:   "agents",
		Short: "list the agents jard can run",
		Args:  cobra.NoArgs,
